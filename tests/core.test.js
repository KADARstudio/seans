...
test('Explicit replacement resets champion (not the seen button)',()=>{let g=C.createGame(pool(6),5);g=C.choose(g,0);g=C.replace(g,0);assert.equal(g.champion,null);assert.equal(g.wins,0);assert.ok(g.pair[0]);});
...
test('Exact provider, genre, rating, duration and explicit unseen filters',()=>{const p=pool(6);const o={services:['netflix'],genre:'scf',runtime:120,rating:7,year:2010,watchState:'new'};assert.deepEqual(C.filterMovies(p,o,['m2']).map(x=>x.id),['m0','m4']);p[0].runtime=null;assert.deepEqual(C.filterMovies(p,o,['m2']).map(x=>x.id),['m4']);});
...
test('Rewatch is enabled by default; filters are explicit',()=>{const p=pool(6);assert.equal(C.filterMovies(p,{services:['netflix']},['m2']).length,3);assert.deepEqual(C.filterMovies(p,{services:['netflix'],watchState:'seen'},['m2']).map(m=>m.id),['m2']);});
