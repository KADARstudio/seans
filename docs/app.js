...
S.set(audioEnabled);
options.watchState=['all','new','seen'].includes(options.watchState)?options.watchState:'all';
...
function isStale(){return offlineCopy||ageHours()>Math.max(3,(catalogue?.refreshHours||1)*1.5);}
...
function statusHTML(){if(!catalogue)return'';return`<div class="status ${isStale()?'stale':''}"><span class="dot"></span><div>${offlineCopy?'Zapisana kopia katalogu. ':''}${ageHours()>Math.max(3,(catalogue?.refreshHours||1)*1.5)?'Dane mogą być nieaktualne. ':''}Oferty PL sprawdzone ${dateText(currentDate())}.<br>${catalogue.movies.length.toLocaleString('pl-PL')} filmów · ${catalogue.coverage?.complete?'pobrano wszystkie strony źródła':'katalog częściowy'}.</div></div>`;}
...
function play(kind){return audioEnabled?S.play(kind):Promise.resolve(false);}
...
async function load(){
  ...
  const boot=c=>{catalogue=c;window.SeansCache?.put(c);options.services=options.services.filter(id=>c.providers.some(p=>p.id===id));screen='home';draw();wireImages();};
  if(window.SEANS_SEED&&validCatalogue(window.SEANS_SEED)){
    ...
    getJSON(endpoints[0]).then(c=>{if(validCatalogue(c)&&Date.parse(c.fetchedAt)>=Date.parse(catalogue.fetchedAt)){
      catalogue=c;offlineCopy=false;window.SeansCache?.put(c);if(screen==='home')draw();
    }}).catch(()=>{});return;
  }
  let data;
  ...
  if(!data){const c=(await window.SeansCache?.get())||read('catalogue',null);if(validCatalogue(c)){data=c;offlineCopy=true;}}
...
function patchDuel(){
  ...
  $('.duel-top').innerHTML=`<span>TWÓJ FILM NA DZIŚ</span><span><strong>${game.completed}</strong> / ${game.goal}</span>`;
...
async function animate(el,frames,duration=440){
...
async function revealWinner(origin){
  ...
  if(clone&&target&&rect.width){
    ...
    try{await animate(clone,[{transform:'translate(0,0) scale(1)'},{transform:`translate(${t.left-rect.left}px,${t.top-rect.top}px) scale(${t.width/rect.width})`}],MOTION.final);}finally{target.style.opacity='';clone.remove();}
  }
  announce(game.champion?`Mamy film: ${movie(game.champion).title}`:'Pula filmów została wyczerpana.');
}
function afterMove(){
...
function start(){
  ...
  // Each session draws a fair sample from the ENTIRE matching catalogue.
  ...
  game=C.createGame(deck,Number(options.rounds));session=L.create();history=[];
...
async function move(kind,side){
  ...
  try{
    if(kind==='pick'){
      ...
      game=C.choose(game,side);play(game.done?'final':'pick');
      ...
      animate(winnerCard,[{transform:'scale(1)'},{transform:'scale(1.015)',offset:.45},{transform:'scale(1)'}],MOTION.select);
      await animate($(`.movie[data-side="${1-side}"]`),[{opacity:1,transform:'translateX(0)'},{opacity:0,transform:`translateX(${side===0?60:-60}px)`}],MOTION.exit);
    }else if(kind==='seen'){
      const id=game.pair[side];session.events.push({type:seen.includes(id)?'unseen':'seen',movie:id,at:Math.round(session.elapsedMs)});
      ...
      await animate($('.duel'),[{opacity:1},{opacity:.3}],MOTION.exit);
    }
    if(game.done){clockAnchor=null;await revealWinner(null);return;}
    patchDuel();persist();play('slide');
    const entrances=[];
    for(const i of [0,1])if(previous[i]!==game.pair[i])entrances.push(animate($(`.movie[data-side="${i}"]`),[{opacity:0,transform:`translateX(${i===0?-18:18}px)`},{opacity:1,transform:'translateX(0)'}],MOTION.enter));
...
async function finish(){
  ...
  play('final');try{await revealWinner($(`.movie[data-movie="${game.champion}"] .poster-wrap`));}finally{moving=false;}
}
function markWatched(id){
  ...
  write('seen',seen);saveJournal();draw();if($('#sheet').open)winnerDetails();notify('Obejrzany. Film nadal może wrócić na kolejny seans.');
}
...
async function importBackup(file){
  ...
  try{
    ...
    options={services:x.options.services,rounds:[10,25,50].includes(Number(x.options.rounds))?Number(x.options.rounds):25,genre:String(x.options.genre||''),runtime:String(x.options.runtime||''),rating:String(x.options.rating||''),year:String(x.options.year||''),watchState:['all','new','seen'].includes(x.options.watchState)?x.options.watchState:'all'};
...
document.addEventListener('change',e=>{const key=e.target.dataset.option;if(key&&['genre','runtime','rating','year','watchState'].includes(key)&&!moving){options[key]=e.target.value;saveOptions();draw();}});
document.addEventListener('click',e=>{
  ...
  if(d.action==='sound'){audioEnabled=!audioEnabled;S.set(audioEnabled);write('sound.v02',audioEnabled);audioButton();if(audioEnabled){play('pick');notify(S.supported()?'Test dźwięku — sprawdź głośność multimediów.':'Ta przeglądarka nie udostępnia dźwięków.');}return;}
  S.unlock();
  ...
  if(d.rate){const r=journal.find(x=>x.id===d.rate),n=Number(d.score);if(r?.watchedAt&&[1,2,3,4,5].includes(n)){r.score=n;saveJournal();draw();if($('#sheet').open)winnerDetails();}return;}
  ...
  switch(d.action){
    ...
    case 'clear-seen':if(confirm('Usunąć oznaczenia obejrzanych? Historia seansów pozostanie.')){seen=[];write('seen',seen);draw();}break;
...
if(S.onError)S.onError(message=>{notify(message);audioButton();});
