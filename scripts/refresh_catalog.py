"""Paginated PL subscription catalogue. Failed refreshes never relabel old data as fresh."""
...
def main():
    ...
    def fetch_provider(provider):
        entries=[];observed=set();cursors=set();after=None;pages=0;expected=0;excluded=0
        while True:
            if pages:time.sleep(1.2)
            data=graphql(QUERY,{'packages':[provider['code']],'after':after})['popularTitles']
            pages+=1;expected=max(expected,int(data['totalCount']))
            for edge in data['edges']:
                n=edge['node'];c=n['content']
                if n['id'] in observed:continue
                observed.add(n['id'])
            ...
            info=data['pageInfo']
            print(provider['name'],'page',pages,'unique',len(observed),'reported',expected,flush=True)
            if not info['hasNextPage']:break
            after=info.get('endCursor')
            if not after or after in cursors or not data['edges']:raise ValueError('Pagination did not advance; previous catalogue unchanged')
            cursors.add(after)
            if pages>=300:raise ValueError('Safety limit reached; refusing to call truncated catalogue complete')
        if not entries:raise ValueError('No verified Polish subscription films for '+provider['name'])
        report={'id':provider['id'],'name':provider['name'],'reportedCount':expected,'fetchedCount':len(observed),'acceptedCount':len(entries),'excludedWithoutOffer':excluded,'pages':pages,'paginationExhausted':True,'complete':len(observed)>=expected and excluded==0}
    ...
    result={'version':1,'country':'PL','monetization':'FLATRATE','fetchedAt':dt.datetime.now(dt.timezone.utc).isoformat(),'fetchStartedAt':started,'refreshHours':6,'source':'JustWatch website (unofficial, non-commercial prototype)','sampleLimitPerProvider':None,'coverage':{'complete':all(p['complete'] for p in reports),'providers':reports,'note':'All exposed pages traversed; upstream availability may still lag platforms.'},'providers':providers,'movies':list(movies.values())}
